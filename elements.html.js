...
export class Element {
  initialize(config = {}) {
    ...
    if (typeof config === "object") {
      if (Array.isArray(config)) {
        // if it is an array, treat it as children (equivalent to { $children: config })
        this.children = config;
      } else if (config instanceof Element) {
        // if it is an element, treat it as a single child
        this.children = config;
      } else {
        // otherwise, it is handled properly as a config object
  ...
  handleObjectConfig(config) {
    ...
    for (let key in config) {
      // Map $children to children property
      if (key === "$children") {
        this.children = config[key];
        continue;
      }
